SELECT mut, ROUND(AVG("score"), 2) AS "ascore" FROM
(WITH t("mut", "id") AS (
...
JOIN (SELECT muts, score FROM runs WHERE ultra IS NOT NULL)
WHERE (muts >> lookup.id) % 2 == 1
GROUP BY mut ORDER BY "ascore" DESC